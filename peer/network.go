...
import (
	...
	"github.com/coinflect/coreth/peer/stats"
...
type network struct {
	...
	outstandingRequestHandlers map[uint32]message.ResponseHandler // maps coinflectchain requestID => message.ResponseHandler
	activeRequests             *semaphore.Weighted                // controls maximum number of active outbound requests
	appSender                  common.AppSender                   // coinflectchain AppSender for sending messages
...
// AppRequest is called by coinflectchain -> VM when there is an incoming AppRequest from a peer
...
// AppRequestFailed can be called by the coinflectchain -> VM in following cases:
// - node is benched
...
// AppGossip is called by coinflectchain -> VM when there is an incoming AppGossip from a peer
// error returned by this function is expected to be treated as fatal by the engine
