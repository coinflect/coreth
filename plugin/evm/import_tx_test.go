...
func TestImportTxVerify(t *testing.T) {
		...
		Outs: []EVMOutput{
			{
				...
				Amount:  importAmount - params.CoinflectAtomicTxFee,
				AssetID: ctx.CFLTAssetID,
			},
			{
	...
	cflt.SortTransferableInputs(importTx.ImportedInputs)
	...
	tests := map[string]atomicTxVerifyTest{
		...
		"inputs sorted incorrectly": {
			generate: func(t *testing.T) UnsignedAtomicTx {
				tx := *importTx
				tx.ImportedInputs = []*cflt.TransferableInput{
					tx.ImportedInputs[1],
					tx.ImportedInputs[0],
				}
				return &tx
				...
				tx.ImportedInputs = []*cflt.TransferableInput{
					{
						...
						Asset: cflt.Asset{ID: ids.GenerateTestID()},
						In: &secp256k1fx.TransferInput{
							Amt: importAmount,
							Input: secp256k1fx.Input{
								SigIndices: []uint32{0},
		...
		"non-CFLT input Banff": {
			...
			ctx:         ctx,
			rules:       banffRules,
			expectedErr: errImportNonCFLTInputBanff.Error(),
		},
		"non-CFLT output Banff": {
			...
			ctx:         ctx,
			rules:       banffRules,
			expectedErr: errImportNonCFLTOutputBanff.Error(),
		},
	}
...
func TestNewImportTx(t *testing.T) {
	...
	// createNewImportCFLTTx adds a UTXO to shared memory and then constructs a new import transaction
	// and checks that it has the correct fee for the base fee that has been used
	createNewImportCFLTTx := func(t *testing.T, vm *VM, sharedMemory *atomic.Memory) *Tx {
		txID := ids.GenerateTestID()
		_, err := addUTXO(sharedMemory, vm.ctx, txID, 0, vm.ctx.CFLTAssetID, importAmount, testShortIDAddrs[0])
		if err != nil {
			t.Fatal(err)
		}
		...
		actualCFLTBurned, err := importTx.Burned(vm.ctx.CFLTAssetID)
		if err != nil {
			t.Fatal(err)
		}
		...
		if actualCFLTBurned != actualFee {
			t.Fatalf("CFLT burned (%d) != actual fee (%d)", actualCFLTBurned, actualFee)
		}
	...
	checkState := func(t *testing.T, vm *VM) {
		...
		tx := txs[0]
		actualCFLTBurned, err := tx.UnsignedAtomicTx.Burned(vm.ctx.CFLTAssetID)
		if err != nil {
			t.Fatal(err)
		}
		...
		expectedRemainingBalance := new(big.Int).Mul(new(big.Int).SetUint64(importAmount-actualCFLTBurned), x2cRate)
		...
		"simple ANT import": {
			UnsignedImportTx: &UnsignedImportTx{
				...
				ImportedInputs: []*cflt.TransferableInput{
					...
					{
						UTXOID: cflt.UTXOID{TxID: ids.GenerateTestID()},
						Asset:  cflt.Asset{ID: antAssetID},
						In: &secp256k1fx.TransferInput{
							Amt:   importAmount,
							Input: secp256k1fx.Input{SigIndices: []uint32{0}},
						},
...
func TestImportTxSemanticVerify(t *testing.T) {
	tests := map[string]atomicTxTest{
		...
		"UTXO AssetID mismatch": {
			setup: func(t *testing.T, vm *VM, sharedMemory *atomic.Memory) *Tx {
				...
				tx := &Tx{UnsignedAtomicTx: &UnsignedImportTx{
					...
					ImportedInputs: []*cflt.TransferableInput{{
						UTXOID: utxo.UTXOID,
						Asset:  cflt.Asset{ID: vm.ctx.CFLTAssetID}, // Use a different assetID then the actual UTXO
						In: &secp256k1fx.TransferInput{
							Amt:   1,
							Input: secp256k1fx.Input{SigIndices: []uint32{0}},
						},
		...
		"insufficient CFLT funds": {
			setup: func(t *testing.T, vm *VM, sharedMemory *atomic.Memory) *Tx {
				txID := ids.GenerateTestID()
				utxo, err := addUTXO(sharedMemory, vm.ctx, txID, 0, vm.ctx.CFLTAssetID, 1, testShortIDAddrs[0])
				if err != nil {
					t.Fatal(err)
				}
		...
		"non-unique EVM Outputs": {
			setup: func(t *testing.T, vm *VM, sharedMemory *atomic.Memory) *Tx {
				txID := ids.GenerateTestID()
				utxo, err := addUTXO(sharedMemory, vm.ctx, txID, 0, vm.ctx.CFLTAssetID, 2, testShortIDAddrs[0])
				if err != nil {
					t.Fatal(err)
				}
	...
	tests := map[string]atomicTxTest{
		"CFLT UTXO": {
			...
			checkState: func(t *testing.T, vm *VM) {
				...
				cfltBalance := sdb.GetBalance(testEthAddrs[0])
				if cfltBalance.Cmp(x2cRate) != 0 {
					t.Fatalf("Expected CFLT balance to be %d, found balance: %d", x2cRate, cfltBalance)
				}
			},
		},
		"non-CFLT UTXO": {
			...
			checkState: func(t *testing.T, vm *VM) {
				...
				cfltBalance := sdb.GetBalance(testEthAddrs[0])
				if cfltBalance.Cmp(common.Big0) != 0 {
					t.Fatalf("Expected CFLT balance to be 0, found balance: %d", cfltBalance)
				}
			},
		},
	}
