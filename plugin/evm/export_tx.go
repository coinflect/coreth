...
var (
	...
	errExportNonCFLTInputBanff                         = errors.New("export input cannot contain non-CFLT in Banff")
	errExportNonCFLTOutputBanff                        = errors.New("export output cannot contain non-CFLT in Banff")
)
...
type UnsignedExportTx struct {
	...
	ExportedOutputs []*cflt.TransferableOutput `serialize:"true" json:"exportedOutputs"`
}
...
func (utx *UnsignedExportTx) Verify(
	...
	for _, in := range utx.Ins {
		...
		if rules.IsBanff && in.AssetID != ctx.CFLTAssetID {
			return errExportNonCFLTInputBanff
		}
	}
	...
	for _, out := range utx.ExportedOutputs {
		...
		assetID := out.AssetID()
		if assetID != ctx.CFLTAssetID && utx.DestinationChain == constants.PlatformChainID {
			return errWrongChainID
		}
		if rules.IsBanff && assetID != ctx.CFLTAssetID {
			return errExportNonCFLTOutputBanff
		}
	}
	if !cflt.IsSortedTransferableOutputs(utx.ExportedOutputs, Codec) {
...
func (utx *UnsignedExportTx) AtomicOps() (ids.ID, *atomic.Requests, error) {
	...
	for i, out := range utx.ExportedOutputs {
		...
		if out, ok := utxo.Out.(cflt.Addressable); ok {
...
func (vm *VM) newExportTx(
	...
	var (
		cfltNeeded           uint64 = 0
		ins, cfltIns         []EVMInput
		signers, cfltSigners [][]*crypto.PrivateKeySECP256K1R
		err                  error
	)
	...
	case rules.IsApricotPhase3:
		...
		cfltIns, cfltSigners, err = vm.GetSpendableCFLTWithFee(keys, cfltNeeded, cost, baseFee)
	default:
		var newCfltNeeded uint64
		newCfltNeeded, err = math.Add64(cfltNeeded, params.CoinflectAtomicTxFee)
		if err != nil {
			return nil, errOverflowExport
		}
		cfltIns, cfltSigners, err = vm.GetSpendableFunds(keys, vm.ctx.CFLTAssetID, newCfltNeeded)
	}
	if err != nil {
		return nil, fmt.Errorf("couldn't generate tx inputs/signers: %w", err)
	}
	ins = append(ins, cfltIns...)
	signers = append(signers, cfltSigners...)

	cflt.SortTransferableOutputs(outs, vm.codec)
...
func (utx *UnsignedExportTx) EVMStateTransfer(ctx *snow.Context, state *state.StateDB) error {
	...
	for _, from := range utx.Ins {
		if from.AssetID == ctx.CFLTAssetID {
			log.Debug("crosschain", "dest", utx.DestinationChain, "addr", from.Address, "amount", from.Amount, "assetID", "CFLT")
			// We multiply the input amount by x2cRate to convert CFLT back to the appropriate
