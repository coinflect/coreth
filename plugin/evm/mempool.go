...
type Mempool struct {
	...
	// CFLTAssetID is the fee paying currency of any atomic transaction
	CFLTAssetID ids.ID
...
func NewMempool(CFLTAssetID ids.ID, maxSize int) *Mempool {
	return &Mempool{
...
// amount of [CFLTAssetID] given the value of [gasUsed].
func (m *Mempool) atomicTxGasPrice(tx *Tx) (uint64, error) {
	...
	burned, err := tx.Burned(m.CFLTAssetID)
	if err != nil {
		return 0, err
	}
