...
func createExportTxOptions(t *testing.T, vm *VM, issuer chan engCommon.Message, sharedMemory *atomic.Memory) []*Tx {
	...
	for _, addr := range testShortIDAddrs {
		exportTx, err := vm.newExportTx(vm.ctx.CFLTAssetID, uint64(5000000), vm.ctx.XChainID, addr, initialBaseFee, []*crypto.PrivateKeySECP256K1R{testKeys[0]})
		if err != nil {
			t.Fatal(err)
		}
...
func TestExportTxEVMStateTransfer(t *testing.T) {
	...
	cfltAmount := 50 * units.MilliCflt
	cfltUTXOID := cflt.UTXOID{
		OutputIndex: 0,
	}
	cfltInputID := cfltUTXOID.InputID()

	customAmount := uint64(100)
	customAssetID := ids.ID{1, 2, 3, 4, 5, 7}
	customUTXOID := cflt.UTXOID{
		OutputIndex: 1,
	}
	customInputID := customUTXOID.InputID()

	customUTXO := &cflt.UTXO{
		UTXOID: customUTXOID,
	...
	tests := []struct {
	...
	}{
		{
			name:        "no transfers",
			tx:          nil,
			cfltBalance: big.NewInt(int64(cfltAmount) * x2cRateInt64),
			balances: map[ids.ID]*big.Int{
		...
		{
			name: "spend half CFLT",
			tx: []EVMInput{
				{
					Address: ethAddr,
					Amount:  cfltAmount / 2,
					AssetID: testCfltAssetID,
					Nonce:   0,
				},
			},
			cfltBalance: big.NewInt(int64(cfltAmount/2) * x2cRateInt64),
			balances: map[ids.ID]*big.Int{
				customAssetID: big.NewInt(int64(customAmount)),
			},
			expectedNonce: 1,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			cfltUTXOBytes, err := vm.codec.Marshal(codecVersion, cfltUTXO)
			if err != nil {
				t.Fatal(err)
			}
			...
			cfltBalance := stateDB.GetBalance(ethAddr)
			if cfltBalance.Cmp(test.cfltBalance) != 0 {
				t.Fatalf("address balance %s equal %s not %s", addr.String(), cfltBalance, test.cfltBalance)
			}
...
func TestExportTxSemanticVerify(t *testing.T) {
	...
	validExportTx := &UnsignedExportTx{
		...
		ExportedOutputs: []*cflt.TransferableOutput{
			{
				Asset: cflt.Asset{ID: custom0AssetID},
				Out: &secp256k1fx.TransferOutput{
					Amt: custom0Balance,
					OutputOwners: secp256k1fx.OutputOwners{
						Threshold: 1,
	...
	validCFLTExportTx := &UnsignedExportTx{
		NetworkID:        vm.ctx.NetworkID,
		BlockchainID:     vm.ctx.ChainID,
		DestinationChain: vm.ctx.XChainID,
		Ins: []EVMInput{
	...
	tests := []struct {
		...
		},
		{
			...
			tx: func() *Tx {
				validExportTx := *validCFLTExportTx
		...
		},
		{
			...
			tx: func() *Tx {
				validExportTx := *validExportTx
				validExportTx.ExportedOutputs = []*cflt.TransferableOutput{{
					Asset: cflt.Asset{ID: custom0AssetID},
					Out: &secp256k1fx.TransferOutput{
						Amt: custom0Balance,
						OutputOwners: secp256k1fx.OutputOwners{
		...
		},
		{
			...
			tx: func() *Tx {
				validExportTx := *validExportTx
				exportOutputs := []*cflt.TransferableOutput{
					{
						Asset: cflt.Asset{ID: custom0AssetID},
						Out: &secp256k1fx.TransferOutput{
				...
				cflt.SortTransferableOutputs(exportOutputs, Codec)
				...
				validExportTx.ExportedOutputs = []*cflt.TransferableOutput{
					{
						Asset: cflt.Asset{ID: vm.ctx.CFLTAssetID},
						Out: &secp256k1fx.TransferOutput{
							Amt: cfltBalance, // after fees this should be too much
							OutputOwners: secp256k1fx.OutputOwners{
								Threshold: 1,
								Addrs:     []ids.ShortID{addr},
							},
...
func TestExportTxAccept(t *testing.T) {
	...
	customUTXOBytes, err := Codec.Marshal(codecVersion, &cflt.UTXO{
		UTXOID: customUTXOID,
		Asset:  cflt.Asset{ID: custom0AssetID},
	...
	cfltUTXOBytes, err := Codec.Marshal(codecVersion, &cflt.UTXO{
		UTXOID: cfltUTXOID,
		Asset:  cflt.Asset{ID: vm.ctx.CFLTAssetID},
	...
	if !bytes.Equal(fetchedValues[1], cfltUTXOBytes) {
		t.Fatalf("incorrect values returned expected %x got %x", cfltUTXOBytes, fetchedValues[1])
	}
}
...
func TestExportTxVerify(t *testing.T) {
	...
	exportTx := &UnsignedExportTx{
		...
		ExportedOutputs: []*cflt.TransferableOutput{
			{
				Asset: cflt.Asset{ID: testCfltAssetID},
				Out: &secp256k1fx.TransferOutput{
					Amt: exportAmount,
					OutputOwners: secp256k1fx.OutputOwners{
						Locktime:  0,
	...
	cflt.SortTransferableOutputs(exportTx.ExportedOutputs, Codec)
	...
	tests := map[string]atomicTxVerifyTest{
		...
		"unsorted outputs": {
			generate: func(t *testing.T) UnsignedAtomicTx {
				tx := *exportTx
				tx.ExportedOutputs = []*cflt.TransferableOutput{
		...
		"invalid exported output": {
			generate: func(t *testing.T) UnsignedAtomicTx {
				tx := *exportTx
				tx.ExportedOutputs = []*cflt.TransferableOutput{tx.ExportedOutputs[0], nil}
				return &tx
			},
			ctx:         ctx,
			rules:       apricotRulesPhase0,
		...
		"non-CFLT input Apricot Phase 6": {
			generate: func(t *testing.T) UnsignedAtomicTx {
				tx := *exportTx
				tx.Ins = []EVMInput{
					{
		...
		"non-CFLT output Apricot Phase 6": {
			generate: func(t *testing.T) UnsignedAtomicTx {
				tx := *exportTx
				tx.ExportedOutputs = []*cflt.TransferableOutput{
					{
						Asset: cflt.Asset{ID: nonExistentID},
						Out: &secp256k1fx.TransferOutput{
							Amt: exportAmount,
							OutputOwners: secp256k1fx.OutputOwners{
								Locktime:  0,
		...
		"non-CFLT input Banff": {
			...
			ctx:         ctx,
			rules:       banffRules,
			expectedErr: errExportNonCFLTInputBanff.Error(),
		},
		"non-CFLT output Banff": {
			generate: func(t *testing.T) UnsignedAtomicTx {
				tx := *exportTx
				tx.ExportedOutputs = []*cflt.TransferableOutput{
					{
			...
			ctx:         ctx,
			rules:       banffRules,
			expectedErr: errExportNonCFLTOutputBanff.Error(),
		},
	}
...
func TestExportTxGasCost(t *testing.T) {
	cfltAssetID := ids.GenerateTestID()
	...
	tests := map[string]struct {
		...
		"simple export 1wei BaseFee": {
			UnsignedExportTx: &UnsignedExportTx{
				...
				ExportedOutputs: []*cflt.TransferableOutput{
					{
						Asset: cflt.Asset{ID: cfltAssetID},
						Out: &secp256k1fx.TransferOutput{
							Amt: exportAmount,
							OutputOwners: secp256k1fx.OutputOwners{
								Locktime:  0,
...
func TestNewExportTx(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			tx, err = vm.newExportTx(vm.ctx.CFLTAssetID, exportAmount, vm.ctx.XChainID, testShortIDAddrs[0], initialBaseFee, []*crypto.PrivateKeySECP256K1R{testKeys[0]})
			if err != nil {
				t.Fatal(err)
			}
			...
			burnedCFLT, err := exportTx.Burned(vm.ctx.CFLTAssetID)
			if err != nil {
				t.Fatal(err)
			}
			if burnedCFLT != test.expectedBurnedCFLT {
				t.Fatalf("burned wrong amount of CFLT - expected %d burned %d", test.expectedBurnedCFLT, burnedCFLT)
			}
			...
			if sdb.GetBalance(addr).Cmp(new(big.Int).SetUint64(test.bal*units.Cflt)) != 0 {
				t.Fatalf("address balance %s equal %s not %s", addr.String(), sdb.GetBalance(addr), new(big.Int).SetUint64(test.bal*units.Cflt))
			}
		})
	}
}
...
func TestNewExportTxMulticoin(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			utxoID2 := cflt.UTXOID{TxID: ids.GenerateTestID()}
			utxo2 := &cflt.UTXO{
				UTXOID: utxoID2,
				Asset:  cflt.Asset{ID: tid},
				Out: &secp256k1fx.TransferOutput{
					Amt: importAmount2,
					OutputOwners: secp256k1fx.OutputOwners{
						Threshold: 1,
			...
			addr := GetEthAddress(testKeys[0])
			if stdb.GetBalance(addr).Cmp(new(big.Int).SetUint64(test.bal*units.Cflt)) != 0 {
				t.Fatalf("address balance %s equal %s not %s", addr.String(), stdb.GetBalance(addr), new(big.Int).SetUint64(test.bal*units.Cflt))
			}
