...
import (
	...
	"github.com/coinflect/coinflectchain/api/keystore"
	...
	"github.com/coinflect/coinflectchain/utils/cb58"
	...
	engCommon "github.com/coinflect/coinflectchain/snow/engine/common"
	...
	accountKeystore "github.com/coinflect/coreth/accounts/keystore"
)
...
var (
	...
	testCfltAssetID  = ids.ID{1, 2, 3}
	username         = "Johns"
	password         = "CjasdjhiPeirbSenfeI13" // #nosec G101
	// Use chainId: 43111, so that it does not overlap with any Coinflect ChainIDs, which may have their
...
func NewContext() *snow.Context {
	...
	ctx.CFLTAssetID = testCfltAssetID
...
func addUTXO(sharedMemory *atomic.Memory, ctx *snow.Context, txID ids.ID, index uint32, assetID ids.ID, amount uint64, addr ids.ShortID) (*cflt.UTXO, error) {
	utxo := &cflt.UTXO{
		UTXOID: cflt.UTXOID{
			TxID:        txID,
			OutputIndex: index,
...
// GenesisVMWithUTXOs creates a GenesisVM and generates UTXOs in the X-Chain Shared Memory containing CFLT based on the [utxos] map
...
func GenesisVMWithUTXOs(t *testing.T, finishBootstrapping bool, genesisJSON string, configJSON string, upgradeJSON string, utxos map[ids.ShortID]uint64) (chan engCommon.Message, *VM, manager.Manager, *atomic.Memory, *engCommon.SenderTest) {
	...
	for addr, cfltAmount := range utxos {
		txID, err := ids.ToID(hashing.ComputeHash256(addr.Bytes()))
		if err != nil {
			t.Fatalf("Failed to generate txID from addr: %s", err)
		}
		if _, err := addUTXO(sharedMemory, vm.ctx, txID, 0, vm.ctx.CFLTAssetID, cfltAmount, addr); err != nil {
...
func TestIssueAtomicTxs(t *testing.T) {
	...
	exportTx, err := vm.newExportTx(vm.ctx.CFLTAssetID, importAmount-(2*params.CoinflectAtomicTxFee), vm.ctx.XChainID, testShortIDAddrs[0], initialBaseFee, []*crypto.PrivateKeySECP256K1R{testKeys[0]})
	if err != nil {
		t.Fatal(err)
	}
...
func TestReissueAtomicTxHigherGasPrice(t *testing.T) {
	...
	for name, issueTxs := range map[string]func(t *testing.T, vm *VM, sharedMemory *atomic.Memory) (issued []*Tx, discarded []*Tx){
		"single UTXO override": func(t *testing.T, vm *VM, sharedMemory *atomic.Memory) (issued []*Tx, evicted []*Tx) {
			utxo, err := addUTXO(sharedMemory, vm.ctx, ids.GenerateTestID(), 0, vm.ctx.CFLTAssetID, units.Cflt, testShortIDAddrs[0])
			if err != nil {
				t.Fatal(err)
			}
			tx1, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], initialBaseFee, kc, []*cflt.UTXO{utxo})
			if err != nil {
				t.Fatal(err)
			}
			tx2, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], new(big.Int).Mul(common.Big2, initialBaseFee), kc, []*cflt.UTXO{utxo})
			if err != nil {
				t.Fatal(err)
			}
		...
		"one of two UTXOs overrides": func(t *testing.T, vm *VM, sharedMemory *atomic.Memory) (issued []*Tx, evicted []*Tx) {
			utxo1, err := addUTXO(sharedMemory, vm.ctx, ids.GenerateTestID(), 0, vm.ctx.CFLTAssetID, units.Cflt, testShortIDAddrs[0])
			if err != nil {
				t.Fatal(err)
			}
			utxo2, err := addUTXO(sharedMemory, vm.ctx, ids.GenerateTestID(), 0, vm.ctx.CFLTAssetID, units.Cflt, testShortIDAddrs[0])
			if err != nil {
				t.Fatal(err)
			}
			tx1, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], initialBaseFee, kc, []*cflt.UTXO{utxo1, utxo2})
			if err != nil {
				t.Fatal(err)
			}
			tx2, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], new(big.Int).Mul(common.Big2, initialBaseFee), kc, []*cflt.UTXO{utxo1})
			if err != nil {
				t.Fatal(err)
			}
		...
		"hola": func(t *testing.T, vm *VM, sharedMemory *atomic.Memory) (issued []*Tx, evicted []*Tx) {
			...
			importTx1, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], initialBaseFee, kc, []*cflt.UTXO{utxo1})
			if err != nil {
				t.Fatal(err)
			}

			importTx2, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], new(big.Int).Mul(big.NewInt(3), initialBaseFee), kc, []*cflt.UTXO{utxo2})
			if err != nil {
				t.Fatal(err)
			}

			reissuanceTx1, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], new(big.Int).Mul(big.NewInt(2), initialBaseFee), kc, []*cflt.UTXO{utxo1, utxo2})
			if err != nil {
				t.Fatal(err)
			}
			...
			reissuanceTx2, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], new(big.Int).Mul(big.NewInt(4), initialBaseFee), kc, []*cflt.UTXO{utxo1, utxo2})
			if err != nil {
				t.Fatal(err)
			}
...
func TestBonusBlocksTxs(t *testing.T) {
	...
	importAmount := uint64(10000000)
	utxoID := cflt.UTXOID{TxID: ids.GenerateTestID()}

	utxo := &cflt.UTXO{
		UTXOID: utxoID,
		Asset:  cflt.Asset{ID: vm.ctx.CFLTAssetID},
		Out: &secp256k1fx.TransferOutput{
			Amt: importAmount,
			OutputOwners: secp256k1fx.OutputOwners{
				Threshold: 1,
...
func TestBuildBlockDoesNotExceedAtomicGasLimit(t *testing.T) {
	...
	for i := 0; i < mempoolTxs; i++ {
		utxo, err := addUTXO(sharedMemory, vm.ctx, txID, uint32(i), vm.ctx.CFLTAssetID, importAmount, testShortIDAddrs[0])
		assert.NoError(t, err)

		importTx, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], initialBaseFee, kc, []*cflt.UTXO{utxo})
		if err != nil {
			t.Fatal(err)
		}
...
func TestExtraStateChangeAtomicGasLimitExceeded(t *testing.T) {
	...
	// in ApricotPhase5.
	for i := 0; i < 100; i++ {
		_, err := addUTXO(sharedMemory1, vm1.ctx, txID, uint32(i), vm1.ctx.CFLTAssetID, importAmount, testShortIDAddrs[0])
		assert.NoError(t, err)

		_, err = addUTXO(sharedMemory2, vm2.ctx, txID, uint32(i), vm2.ctx.CFLTAssetID, importAmount, testShortIDAddrs[0])
		assert.NoError(t, err)
	}
