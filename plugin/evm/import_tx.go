...
var (
	...
	errImportNonCFLTInputBanff                         = errors.New("import input cannot contain non-CFLT in Banff")
	errImportNonCFLTOutputBanff                        = errors.New("import output cannot contain non-CFLT in Banff")
)
...
func (utx *UnsignedImportTx) Verify(
	...
	for _, out := range utx.Outs {
		...
		if rules.IsBanff && out.AssetID != ctx.CFLTAssetID {
			return errImportNonCFLTOutputBanff
		}
	}
	...
	for _, in := range utx.ImportedInputs {
		...
		if rules.IsBanff && in.AssetID() != ctx.CFLTAssetID {
			return errImportNonCFLTInputBanff
		}
	}
	if !cflt.IsSortedAndUniqueTransferableInputs(utx.ImportedInputs) {
...
func (utx *UnsignedImportTx) SemanticVerify(
	...
	case rules.IsApricotPhase3:
		...
		fc.Produce(vm.ctx.CFLTAssetID, txFee)

	// Apply fees to import transactions as of Apricot Phase 2
	case rules.IsApricotPhase2:
		fc.Produce(vm.ctx.CFLTAssetID, params.CoinflectAtomicTxFee)
	}
	for _, out := range utx.Outs {
...
func (vm *VM) newImportTxWithUTXOs(
	...
	atomicUTXOs []*cflt.UTXO, // UTXOs to spend
) (*Tx, error) {
	...
	for _, utxo := range atomicUTXOs {
		...
		input, ok := inputIntf.(cflt.TransferableIn)
		if !ok {
			continue
		}
		aid := utxo.AssetID()
		...
		importedInputs = append(importedInputs, &cflt.TransferableInput{
			UTXOID: utxo.UTXOID,
			Asset:  utxo.Asset,
			In:     input,
		})
	...
	cflt.SortTransferableInputsWithSigners(importedInputs, signers)
	importedCFLTAmount := importedAmount[vm.ctx.CFLTAssetID]
	...
	for assetID, amount := range importedAmount {
		// Skip the CFLT amount since it is included separately to account for
		// the fee
		if assetID == vm.ctx.CFLTAssetID || amount == 0 {
			continue
		}
	...
	case rules.IsApricotPhase2:
		txFeeWithoutChange = params.CoinflectAtomicTxFee
		txFeeWithChange = params.CoinflectAtomicTxFee
	}

	// CFLT output
	if importedCFLTAmount < txFeeWithoutChange { // imported amount goes toward paying tx fee
		return nil, errInsufficientFundsForFee
	}

	if importedCFLTAmount > txFeeWithChange {
		outs = append(outs, EVMOutput{
			Address: to,
			Amount:  importedCFLTAmount - txFeeWithChange,
			AssetID: vm.ctx.CFLTAssetID,
		})
	}

	// If no outputs are produced, return an error.
	// Note: this can happen if there is exactly enough CFLT to pay the
...
func (utx *UnsignedImportTx) EVMStateTransfer(ctx *snow.Context, state *state.StateDB) error {
	for _, to := range utx.Outs {
		if to.AssetID == ctx.CFLTAssetID {
			log.Debug("crosschain", "src", utx.SourceChain, "addr", to.Address, "amount", to.Amount, "assetID", "CFLT")
			// If the asset is CFLT, convert the input amount in nCFLT to gWei by
