...
import (
	...
	coinflectchainMetrics "github.com/coinflect/coinflectchain/api/metrics"

	"github.com/coinflect/coreth/consensus/dummy"
	corethConstants "github.com/coinflect/coreth/constants"
	"github.com/coinflect/coreth/core"
	"github.com/coinflect/coreth/core/rawdb"
	"github.com/coinflect/coreth/core/state"
	"github.com/coinflect/coreth/core/types"
	"github.com/coinflect/coreth/eth"
	"github.com/coinflect/coreth/eth/ethconfig"
	"github.com/coinflect/coreth/ethdb"
	corethPrometheus "github.com/coinflect/coreth/metrics/prometheus"
	"github.com/coinflect/coreth/miner"
	"github.com/coinflect/coreth/node"
	"github.com/coinflect/coreth/params"
	"github.com/coinflect/coreth/peer"
	"github.com/coinflect/coreth/plugin/evm/message"
	"github.com/coinflect/coreth/rpc"
	statesyncclient "github.com/coinflect/coreth/sync/client"
	"github.com/coinflect/coreth/sync/client/stats"
	"github.com/coinflect/coreth/sync/handlers"
	handlerstats "github.com/coinflect/coreth/sync/handlers/stats"
	...
	// inside of cmd/geth.
	_ "github.com/coinflect/coreth/eth/tracers/native"
	...
	coinflectRPC "github.com/gorilla/rpc/v2"
	...
	"github.com/coinflect/coinflectchain/utils/perms"
	"github.com/coinflect/coinflectchain/utils/profiler"
	...
	commonEng "github.com/coinflect/coinflectchain/snow/engine/common"

	coinflectJSON "github.com/coinflect/coinflectchain/utils/json"
)
...
var (
	...
	// 1 nCFLT and the smallest denomination on the C-Chain 1 wei. Where 1 nCFLT = 1 gWei.
	// This is only required for CFLT because the denomination of 1 CFLT is 9 decimal
...
var (
	...
	errInsufficientFundsForFee        = errors.New("insufficient CFLT funds to pay transaction fee")
...
type VM struct {
	...
	*chain.State
	...
	// Metrics
	multiGatherer coinflectchainMetrics.MultiGatherer
...
func (vm *VM) Initialize(
	...
	// Set the chain config for mainnet/addismya chain IDs
	switch {
	case g.Config.ChainID.Cmp(params.CoinflectMainnetChainID) == 0:
		g.Config = params.CoinflectMainnetChainConfig
		extDataHashes = mainnetExtDataHashes
	case g.Config.ChainID.Cmp(params.CoinflectAddismyaChainID) == 0:
		g.Config = params.CoinflectAddismyaChainConfig
		extDataHashes = addismyaExtDataHashes
	case g.Config.ChainID.Cmp(params.CoinflectLocalChainID) == 0:
		g.Config = params.CoinflectLocalChainConfig
	}
	// Set the Coinflect Context on the ChainConfig
	g.Config.CoinflectContext = params.CoinflectContext{
	...
	if g.Config.ChainID.Cmp(params.CoinflectLocalChainID) != 0 {
	...
	// config, free addismya)
	addismyaExtDataHashes = nil
	...
	vm.mempool = NewMempool(chainCtx.CFLTAssetID, defaultMempoolSize)
	...
	if vm.chainID.Cmp(params.CoinflectMainnetChainID) == 0 {
...
func (vm *VM) initializeMetrics() error {
	vm.multiGatherer = coinflectchainMetrics.NewMultiGatherer()
...
func (vm *VM) preBatchOnFinalizeAndAssemble(header *types.Header, state *state.StateDB, txs []*types.Transaction) ([]byte, *big.Int, *big.Int, error) {
	for {
		...
		rules := vm.chainConfig.CoinflectRules(header.Number, new(big.Int).SetUint64(header.Time))
		...
		if rules.IsApricotPhase4 {
			contribution, gasUsed, err = tx.BlockFeeContribution(rules.IsApricotPhase5, vm.ctx.CFLTAssetID, header.BaseFee)
			if err != nil {
...
func (vm *VM) postBatchOnFinalizeAndAssemble(header *types.Header, state *state.StateDB, txs []*types.Transaction) ([]byte, *big.Int, *big.Int, error) {
	...
	for {
		...
		// ApricotPhase5.
		txContribution, txGasUsed, err = tx.BlockFeeContribution(true, vm.ctx.CFLTAssetID, header.BaseFee)
		if err != nil {
			return nil, nil, nil, err
		}
...
func (vm *VM) onExtraStateChange(block *types.Block, state *state.StateDB) (*big.Int, *big.Int, error) {
	...
	for _, tx := range txs {
		...
		if rules.IsApricotPhase4 {
			contribution, gasUsed, err := tx.BlockFeeContribution(rules.IsApricotPhase5, vm.ctx.CFLTAssetID, block.BaseFee())
			if err != nil {
				return nil, nil, err
			}
...
func newHandler(name string, service interface{}, lockOption ...commonEng.LockOption) (*commonEng.HTTPHandler, error) {
	server := coinflectRPC.NewServer()
	server.RegisterCodec(coinflectJSON.NewCodec(), "application/json")
	server.RegisterCodec(coinflectJSON.NewCodec(), "application/json;charset=UTF-8")
...
func (vm *VM) CreateHandlers(context.Context) (map[string]*commonEng.HTTPHandler, error) {
	...
	cfltAPI, err := newHandler("cflt", &CfltAPI{vm})
	if err != nil {
		return nil, fmt.Errorf("failed to register service for CFLT API due to %w", err)
	}
	enabledAPIs = append(enabledAPIs, "cflt")
	apis[cfltEndpoint] = cfltAPI
...
func (vm *VM) GetAtomicUTXOs(
	...
	startUTXOID ids.ID,
	limit int,
) ([]*cflt.UTXO, ids.ShortID, ids.ID, error) {
	...
	utxos := make([]*cflt.UTXO, len(allUTXOBytes))
...
func (vm *VM) GetSpendableFunds(
	...
	for _, key := range keys {
		...
		var balance uint64
		if assetID == vm.ctx.CFLTAssetID {
			// If the asset is CFLT, we divide by the x2cRate to convert back to the correct
			// denomination of CFLT that can be exported.
...
// GetSpendableCFLTWithFee returns a list of EVMInputs and keys (in corresponding
// order) to total [amount] + [fee] of [CFLT] owned by [keys].
...
// corresponds to a single key, so that the signers can be passed in to
// [tx.Sign] which supports multiple keys on a single input.
func (vm *VM) GetSpendableCFLTWithFee(
	keys []*crypto.PrivateKeySECP256K1R,
	amount uint64,
	cost uint64,
	baseFee *big.Int,
	...
	for _, key := range keys {
		...
		addr := GetEthAddress(key)
		// Since the asset is CFLT, we divide by the x2cRate to convert back to
		// the correct denomination of CFLT that can be exported.
		balance := new(big.Int).Div(state.GetBalance(addr), x2cRate).Uint64()
...
func (vm *VM) currentRules() params.Rules {
	...
	return vm.chainConfig.CoinflectRules(header.Number, big.NewInt(int64(header.Time)))
}
