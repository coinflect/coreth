...
// BlockFeeContribution calculates how much CFLT towards the block fee contribution was paid
// for via this transaction denominated in [cfltAssetID] with [baseFee] used to calculate the
// cost of this transaction. This function also returns the [gasUsed] by the
// transaction for inclusion in the [baseFee] algorithm.
func (tx *Tx) BlockFeeContribution(fixedFee bool, cfltAssetID ids.ID, baseFee *big.Int) (*big.Int, *big.Int, error) {
	if baseFee == nil {
	...
	burned, err := tx.Burned(cfltAssetID)
	if err != nil {
		return nil, nil, err
	}
	if txFee > burned {
		return nil, nil, fmt.Errorf("insufficient CFLT burned (%d) to cover import tx fee (%d)", burned, txFee)
	}
	excessBurned := burned - txFee

	// Calculate the amount of CFLT that has been burned above the required fee denominated
...
// calculates the amount of CFLT that must be burned by an atomic transaction
...
func calculateDynamicFee(cost uint64, baseFee *big.Int) (uint64, error) {
	...
	feeInNCFLT := new(big.Int).Div(feeToRoundUp, x2cRate)
	if !feeInNCFLT.IsUint64() {
		// the fee is more than can fit in a uint64
		return 0, errFeeOverflow
	}
	return feeInNCFLT.Uint64(), nil
}
